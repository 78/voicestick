...
int ClampConfirmationCountdownMs(int ms) {
    return std::clamp(ms, 0, 10000);
}
...
std::filesystem::path AppConfig::ConfigPath() {
    return ConfigDirectory() / "config.toml";
}

std::filesystem::path AppConfig::DefaultDebugAudioDirectory() {
    return XdgDirectory("XDG_DATA_HOME", ".local/share") / "DebugAudio";
}
...
std::string FormatPairedDeviceEntry(const PairedDeviceEntry& entry) {
    char addr_buf[17]{};
    snprintf(addr_buf, sizeof(addr_buf), "%012llX",
             static_cast<unsigned long long>(entry.bluetooth_address));
...
void ApplyConfigValue(AppConfig& config, const std::string& key, const std::string& value) {
    ...
    if (key == "confirmation_countdown_ms") {
        config.confirmation_countdown_ms = ClampConfirmationCountdownMs(
            IntValue(value, config.confirmation_countdown_ms));
    }
...
AppConfig AppConfig::Load() {
    ...
    try {
        ...
        if (auto value = TomlInt(table, "confirmation_countdown_ms")) {
            config.confirmation_countdown_ms = ClampConfirmationCountdownMs(*value);
        }
...
void AppConfig::Save() const {
    ...
    output << "confirmation_countdown_ms = " << confirmation_countdown_ms << "\n";
...
void AppConfig::SavePairedDevice(const PairedDeviceEntry& entry) {
    ...
    if (std::find(paired_device_ids.begin(), paired_device_ids.end(), entry.device_id) ==
        paired_device_ids.end()) {
