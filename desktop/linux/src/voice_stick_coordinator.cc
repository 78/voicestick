...
void VoiceStickCoordinator::FinishWithFinalText(const std::string& text) {
    ...
    if (profile.transform == TextTransform::kTranslate) {
        ...
        TransformText(text, profile, [this](bool ok, std::string result) {
            if (ok) {
                ...
                if (pause_confirmation_on_final_) {
                    pause_confirmation_on_final_ = false;
                    EnterPausedConfirmation(result, "disconnect_paused");
                } else {
    ...
    if (pause_confirmation_on_final_) {
        pause_confirmation_on_final_ = false;
        EnterPausedConfirmation(text, "disconnect_paused");
        return;
    }
...
void VoiceStickCoordinator::CancelActiveCycleIfDeviceDisconnected() {
    ...
    if (active_device_id_.has_value() && !ble_->IsConnected(*active_device_id_)) {
        if (waiting_for_audio_end_.load() || session_state_ == SessionState::kRecording) {
            LogCoordinatorLine("device disconnected; finalize buffered audio");
            pause_confirmation_on_final_ = true;
            SendFinalOggChunkIfNeeded(CurrentRecordingDurationSeconds());
            return;
        }
        if (!pending_paste_state_.IsIdle() || session_state_ == SessionState::kFinalizing) {
            LogCoordinatorLine("device disconnected; keep recognized text");
            return;
        }
        asr_->Cancel();
        pending_paste_state_ = {};
...
void VoiceStickCoordinator::EnterPendingConfirmation(const std::string& text, std::string_view reason) {
    ...
    SendUiStateForActiveDevice("pending_confirmation");
    if (config_.confirmation_countdown_ms <= 0) {
        ui_->HideOverlay([this, text] { CommitPendingPaste(text); });
        return;
    }
...
void VoiceStickCoordinator::RefreshDeviceUiState(const std::string& device_id) {
    ...
    case SessionState::kPausedConfirmation:
        if (active_device_id_ == device_id) {
            ble_->SendUiState("pending_confirmation", "", device_id);
        } else {
            ble_->SendUiState("ready", "", device_id);
        }
        break;
